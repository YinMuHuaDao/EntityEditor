...
-- taskParameters.pattern_type Type: Integer (0 - choice limit)
-- taskParameters.length Type: Real Unit: meters
-- taskParameters.width Type: Real Unit: meters
...
function determineTurnRate(targetHeading)
   --If we are using a custom pattern will need to alter our turn rate on the curves. Currently all planes
   -- use the default
   local turnTime = 3.0
   if ( taskParameters.pattern_type > 0 ) then
      turnTime = ((taskParameters.width)*math.pi)/(2.0*taskParameters.speed)
   else
      turnTime  = taskParameters.timeValue
   end
   
   if ( turnTime > 0.0 ) then
      return math.pi/turnTime
   else
      return 0.0
   end
end
...
function startDirectionalTurn(targetHeading)
   if ( this:getSpeed() > 0.0) then
      local turnRate = determineTurnRate(targetHeading)
      mySubtaskId = vrf:startSubtask("fly-heading-task", { heading = targetHeading,turn_rate = turnRate, turn_direction = direction });
   end
end
...
function startReverseTurn(targetHeading)
   if ( this:getSpeed() > 0.0) then
      local turnRate = determineTurnRate(targetHeading)
      mySubtaskId = vrf:startSubtask("fly-heading-task", { heading = targetHeading,turn_rate = turnRate, turn_direction =-1 *  direction });
   end
end
...
function startWait(duration)
   ...
   if ( taskParameters.pattern_type > 0 and this:getSpeed() > 0.0) then
      deltaT = taskParameters.length/this:getSpeed()
   else
      deltaT = taskParameters.timeValue
   end
   myWaitExpires = vrf:getExerciseTime() +deltaT
...
function showTaskVisualizations()
   ...
   local legLength = 60.0
   
   -- Length of time in seconds to fly each leg
   if ( taskParameters.pattern_type==1 and taskParameters.speed > 0.0) then
      legLength = taskParameters.length/taskParameters.speed
   else
      legLength = taskParameters.timeValue
   end
   ...
   -- Default turn rate of fly heading task is 3 degrees/sec.
	...
	local turnTime  = legLength
   --Determine pattern type
   if ( taskParameters.pattern_type > 0 and taskParameters.speed > 0.0 ) then
      local t = (math.pi * (taskParameters.width/2.0))/taskParameters.speed
      turnTime = t
      turnRate = math.pi/t
      
   else
      turnTime = taskParameters.timeValue
      turnRate = math.pi/turnTime
   end
