...
local theAnimations = { 
   {animation = "angry_yell"},
   {animation = "angry_chant_1_arm"},
   {animation = "angry_chant_2_arms"},
...
function startAnimation()
   local numTries = 0
   while (numTries < #theAnimations * 2) do
      local entry = theAnimations[math.random(#theAnimations)]
...
function tick()
   if (mySubtaskId >= 0) then
      if (vrf:isSubtaskRunning(mySubtaskId) == false) then
	     vrf:cleanupInactiveSubtaskStatus()
         startAnimation()
      end
   end
end
