   ...
   if (waypoint:isValid()) then
      ...
      elseif (not vrf:isSubtaskRunning(mySubtaskId)) then
	    ...
	    errorExit("Subtask is not running and was not completed.")
         ...
         -- do a distance check rather than an equality check to work around potential precision issues.  Use tolerance of 1 meter
         if (myStoredLocation:distanceToLocation3D(waypoint:getLocation3D()) > 1) then
