...
function targetLocation()
   if (not taskParameters.targetName:isValid()) then
      return nil
   end
...
function moveToLAR()
   ...
   startLoc:setAlt(0)
   local targetLocOnGround = targetLocationOnGround()

   if (targetLocOnGround == nil) then
      return false
   end

   local startToEnd = startLoc:vectorToLoc3D(targetLocOnGround)
