...
--  taskParameters.TargetLocation: location3D - location to throw at
--  taskParameters.MunitionType Type: String (resource name) - Type of grenade to throw
...
function tick()
   ...
   elseif ( checkpointState.myState == "stowweapon" ) then
      if (vrf:isSubtaskComplete(checkpointState.myTaskId) ) then
         ...
         checkpointState.myLocation = taskParameters.TargetLocation
   ...
   elseif ( checkpointState.myState == "turning"  ) then
      ...
      if (vrf:isSubtaskComplete(checkpointState.myTaskId) ) then
         checkpointState.myTaskId = vrf:startSubtask("Fire_For_Effect_on_Location",{location=checkpointState.myLocation,munition=taskParameters.MunitionType,numRnds =1,heightAboveTerrain = 0.0})
