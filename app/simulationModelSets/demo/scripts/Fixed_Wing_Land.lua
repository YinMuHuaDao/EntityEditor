...
-- For detailed, tick-level debugging
DEBUG = false
...
local heightAboveRunway = 0 - this:getParameter("right-support"):getUp()
...
function createApproachRoute()
   ...
   local approachStartVec = runwayVec:getNegated()
    -- Distance from end of runway to start descent
   approachStartVec:setBearingInclRange(approachStartVec:getBearing(), 0, 3300)
   approachStartVec:setNorthEastDown(approachStartVec:getNorth(), 
      approachStartVec:getEast(), -300 - heightAboveRunway)
   approachStart = runwayPoints[1]:addVector3D(approachStartVec)
    -- Distance from end of runway to level the descent path
   approachStartVec:setBearingInclRange(approachStartVec:getBearing(), 0, 350)
   approachStartVec:setNorthEastDown(approachStartVec:getNorth(), 
      approachStartVec:getEast(), -17 - heightAboveRunway)
   local approachLevel = runwayPoints[1]:addVector3D(approachStartVec)
    -- Distance from end of runway to end descent path
   approachStartVec:setBearingInclRange(approachStartVec:getBearing() + math.rad(180), 0, 100)
   approachStartVec:setNorthEastDown(approachStartVec:getNorth(), 
      approachStartVec:getEast(), -17 - heightAboveRunway)
   ...
   approachRoute = vrf:createRoute({locations = routePoints, entity_type = "17:0:0:2:32:0:0", 
      publish = DEBUG, object_name = "Landing Approach" })
...
function approachSpeed()
   -- Max speed for landing is 1.1 * min-speed
   return this:getParameter("min-speed") * 1.05
end
...
function startTouchdownAnimation()
   if (DEBUG) then
      luaPrint("  Starting touchdown animation")
   end
   local runwayPoints = taskParameters.runway:getLocations3D()
   local runwayVec = runwayPoints[1]:vectorToLoc3D(runwayPoints[2])
   ...
   mySubtaskId = vrf:startSubtask("animated-movement-task", { 
      animation_model = "Fixed Wing Landing", reference_heading = runwayVec:getBearing(),
...
function tick()
   if (DEBUG) then
      luaPrint(string.format("\n%.3f %s FW Land- state %s",
         vrf:getSimulationTime(), this:getName(),
	 myPhase))
   end
   ...
   if (myPhase == "approach") then
      ...
      if (DEBUG) then
         luaPrint("  Range to first runway point ", offsetVec:getRange(),
            " (threshold 215)")
      end
      ...
      if (not myLandingSpeedSet) then
         ...
         if (distToApproach < decelerationDistance()) then
            ...
            if (DEBUG) then
               luaPrint("  Setting approach speed to ", approachSpeed())
            end
         end			
      end
      ...
      if (not myGearDown) then
         local distToApproach = this:getLocation3D():vectorToLoc3D(
            taskParameters.runway:getLocations3D()[1]):getRange()
         if (distToApproach < 1000) then 
            landingGearDown()
            myGearDown = true
            if (DEBUG) then
               luaPrint("  Distance to runway < 1km, lowering gear")
          luaPrint("  Distance to runway < 1km, lowering gear")
            end
         end
      end
   ...
   if (myPhase == "just-embarked") or (vrf:isSubtaskComplete(mySubtaskId)) then
      ...
      elseif ((myPhase == "touch-down") or (myPhase == "just-embarked")) then	
         ...
         if (DEBUG) then
            luaPrint("  Clamping to terrain surface")
         end
         local alt, valid  = vrf:getTerrainAltitude(this:getLocation3D():getLat(), 
   ...
   elseif ((myPhase == "touch-down") and taskParameters.runway:isEmbarked() and 
      this:isOnTopOf(taskParameters.runway:getEmbarkedOn())) then
      
      -- Stop the animation at this point and continue with the move to the last point
      if (DEBUG) then
         luaPrint("  Embarked and on top of parent; stopping animation")
      end
      vrf:stopSubtask(mySubtaskId)
