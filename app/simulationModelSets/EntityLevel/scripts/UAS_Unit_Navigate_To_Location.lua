...
function init()
   ...
   if (destination == nil) then
      printWarn("UAS_Unit_Navigate_To_Location(): no destination?\n")
      vrf:endTask(false)
      return
   end
...
function taskSubordinateMovement()
   ...
   for idx,sub in ipairs(subordinates) do
      ...
      if (sub:isDestroyed() == false) and (subDest ~= nil) then
         local params = {
            destination = subGoals[sub:getUUID()],
         ...
         local taskId = vrf:sendTask(sub, "rotary-navigate-to-location", params)
