...
function indexForSub(subordinate)
   for i, sub in pairs(this:getSubordinates(true)) do 
...
function getSubIndex(movementIndex)
   ...
   return #this:getSubordinates(true) - (movementIndex - 1)
end
...
function getFirstSubIndex()
   ...
   return #this:getSubordinates(true)
end
...
function taskSub(subordinate)
   ...
   --print("Offset: ", offset)
   offset = offset:getScaled(1/(#this:getSubordinates(true) - 1))
   --print("Offset: ", offset) 
...
function finishSubMove(subIndex)
   ...
   if guard then
        setSubToGuard(this:getSubordinates(true)[subIndex])          	
   else
       local coverObject = taskParameters.coverObjects[currentCoverIndex]
       local lineStart = coverObject:getLocations3D()[1]
       local lineEnd = coverObject:getLocations3D()[2]
       local offset = lineEnd:vectorToLoc3D(lineStart)
       vrf:sendSetData(this:getSubordinates(true)[subIndex], "set-heading", {heading = offset:getBearing()})
	   if (subIndex == getFirstSubIndex()) then
		  vrf:sendSetData(this:getSubordinates(true)[subIndex], "set-lifeform-weapon-state", {lifeform_weapon_state = "weapon-in-fire-position"})
	   end 
	       --print ("not first sub index", movingSubIndex, ", ", getFirstSubIndex(), ", ", this:getSubordinates(true)[getSubIndex(movingSubIndex)])  
   end
end
...
function startMoveLeg()
   ...
   if (currentCoverIndex <= #taskParameters.coverObjects) then
	   --print ("SUB: ", movingSubIndex, ", ", getSubIndex(movingSubIndex), ", ", this:getSubordinates(true)[getSubIndex(movingSubIndex)])
	   taskSub(this:getSubordinates(true)[getSubIndex(movingSubIndex)])
   else
...
function tick()   
   if (checkSubordinateTasks() == 0) then      
      if (movingSubIndex > #this:getSubordinates(true) or movingSubIndex < 1) then
         startMoveLeg()
      else        
         taskSub(this:getSubordinates(true)[getSubIndex(movingSubIndex)])
      end
