...
-- If the entity becomes suppressed to level 2 or 3, the entity stops and falls prone.
-- The reaction is not ended until the suppression level goes back to 0.
...
checkpointState.oldPosture = nil
...
function check()
   ...
   return (aiEnabled == nil or aiEnabled) and 
      this:getSuppressionLevel() > 1
end
...
function init()
   checkpointState.oldPosture = this:getLifeformPosture()
   targetPosture = LifeformPosture.StringToEnum("prone")
...
function tick()
   if this:getSuppressionLevel() == 0 then
      ...
      vrf:executeSetData("set-lifeform-posture", {lifeform_posture = checkpointState.oldPosture})
      vrf:endTask(true)
   end
end
