...
local searchInterval = 20;  -- Number of seconds between attempts to locate a nearby friend.
...
function startWander()
    wanderSubtaskId = vrf:startSubtask("wander", {area=wanderArea,movementMode=movement})
end
...
function moveToFriend()
   ...
   if (moveSubtaskId < 0) then
      -- pick a meeting point 1/2 between the two parties. 
      local delta = this:getLocation3D():vectorToLoc3D(myFriend:getLocation3D())
      local a = Vector3D(0,0,0)
      a:setBearingInclRange(delta:getBearing(),0,delta:getRange()/2)
      moveSubtaskId = vrf:startSubtask("move-to-location", {aiming_point=this:getLocation3D():addVector3D(a)})
   end
...
-- Turn to the friend and wait for them to come over. 
function nearFriend()

   local delta = this:getLocation3D():vectorToLoc3D(myFriend:getLocation3D())
   if (delta:getRange() < 2.0) then
	this:setHeading(delta:getBearing());
	return true;
   else
      if (facingFriendId < 0) then
         facingFriendId = vrf:startSubtask("move-to", {destination=myFriend})
      end
   -- Check the state of an already started movement task.
      if (vrf:isSubtaskComplete(facingFriendId)) then
          facingFriendId = -1;
          return true;
      end
   end
