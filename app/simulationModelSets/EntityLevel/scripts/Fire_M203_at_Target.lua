...
--  taskParameters.Target Type: SimObject - Target to Fire at
--  taskParameters.MunitionType Type: String (weapon name) - Type of grenade to fire
...
function tick()
   ...
   if ( myState == "start") then
      ...
      myLocation = taskParameters.Target:getLocation3D()
   ...
   elseif ( myState == "turning"  ) then
      ...
      if (vrf:isSubtaskComplete(myTaskId) ) then
         myTaskId = vrf:startSubtask("Fire_For_Effect_on_Target",{target=taskParameters.Target,munition=taskParameters.MunitionType,numRnds =1,heightAboveTerrain = 0.0})
