...
-- by inclusion in a weapon system that uses anti-air missile munitions (munitions must
-- have "Missile" in their system name).
-- When activated, it will look for threats within the given range
-- and engage them with an anti-air missile weapon that can be used in the 
-- current postures, and has the range and ceiling.
--
...
local WEAPON_CATEGORY = "Anti-Air"
local TARGET_TYPES = {EntityType.AggregateAir()}
...
-- with {range, strength, ceiling} in each entry.
...
function getValidWeapons()
   ...
   if allIFWeapons == nil or
      ...
      printWarn(vrf:trUtf8("Error: React to Air Threats: Parameter property(s) nil"))
      return validWeapons
   end
         ...
         if weapRange ~= nil then
            ...
            if DEBUG_DETAIL then
               printDebug(".  Adding ", weapon, ", range ", 
                  weapRange, ", ceiling ", weapCeiling,
                  ", strength ", weapStrength)
            end
         end
      end
   end
...
--Returns a list of weapons that are ready, plus a string with possible
-- reasons for weapons not being ready.
function findReadyWeapons(weaponList, myPosture)
   ...
   for name, info in pairs(weaponList) do
      ...
      if hasAmmo(name) then
         if postureAllowsWeaponUse(weapProperty, myPosture) then
            ready[name] = info
         else 
            reason = "Posture does not allow use of weapon."
         end
      else
         reason = "Weapon out of ammo."
      end
...
function canEngageBandit(bandit, readyWeapons)
   ...
   local banditAlt = banditLoc:getAlt()
   local myAlt = this:getLocation3D():getAlt()
   ...
   for weapon, weaponInfo in pairs(readyWeapons) do
      if DEBUG_DETAIL_CHECK then
         ...
         ", ceiling ", weaponInfo.altitude)
      end
      -- Check range and altitude.
      if banditRange <= weaponInfo.range and
         banditAlt <= weaponInfo.ceiling
         then
      ...
      elseif DEBUG_DETAIL then
         ...
         if banditAlt > weaponInfo.ceiling then
            printDebug(".  .  Target alt ", banditAlt,
               " is too high.")
         end
      end
   end
...
function checkInit()
   ...
   if DEBUG_DETAIL then
      printDebug(string.format("\n%.3f React to Air Threat: checkInit",
...
function check()
   if DEBUG_DETAIL_CHECK then
      printDebug(string.format("\n%.3f React to Air Threat: checking for threats.",
         vrf:getSimulationTime()))
   end
   if this:getEmbarkedOn():isValid() then
      printWarn(vrf:trUtf8("React to Air Threat: Cannot attack while embarked."))
      return false
   end
   ...
   if validWeaponList == nil then
      ...
      if next(validWeaponList) == nil then
         printWarn(vrf:trUtf8("React to Air Threat: No valid anti-air weapons."))
   ...
   local reason
   readyWeapons, reason = findReadyWeapons(validWeaponList, this:getStateProperty("Posture"))
   -- Print out a warning message once, until the state changes.
   if next(readyWeapons) == nil then
      if weaponsReady then
         printWarn(vrf:trUtf8("React to Air Threat: No anti-air missiles ready. %1"):
            arg(reason))
         weaponsReady = false
      end
      return false
      ...
      if not weaponsReady then
         printWarn(vrf:trUtf8("React to Air Threat: Anti-air missiles ready."))
         weaponsReady = true
      end
   end
         ...
         if not contact:isDestroyed() then
            ...
            if matchesTargetTypes(contactType) then
               ...
               if canEngage then
                  ...
                  if DEBUG_DETAIL_CHECK then
                     printDebug(".   Range ", range, ", altitude ", altitude,
...
function init()
   if DEBUG_DETAIL then
      printDebug("\nReact to Air Threat: init reaction")
   end
...
function tick()
   ...
   if attackSubtaskId == -1 then
      ...
      if DEBUG_DETAIL then
         printDebug(string.format("\n%.3f React to Air Threat: attacking threat %s with %s",
