...
-- by inclusion in a weapon system that uses missile munitions (munitions must
-- have "Missile" in their system name).
-- When activated, it will look for threats within the given range
-- and engage them with an anti-ship missile weapon that can be used in the 
-- current postures, and has the range. Since the weapon is a missile, it is assumed
...
local TARGET_TYPES = {EntityType.AggregateSurface()}
-- Note also in this script, the weapon name must have "Missile" in it.
...
function getValidWeapons()
   ...
   if allIFWeapons == nil or
      ...
      printWarn(vrf:trUtf8("Error: React to Naval with Missile: Parameter property(s) nil"))
      return validWeapons
   end
...
function canEngageBandit(bandit, readyWeapons)
   ...
   local terrainAlt = vrf:getTerrainAltitude(banditLoc:getLat(),
      banditLoc:getLon())
   ...
   for weapon, weaponInfo in pairs(readyWeapons) do
      ...
      elseif DEBUG_DETAIL then
         ...
         if banditAlt - terrainAlt < -0.5 then
            printDebug(".  .  Target not on surface")
         end
      end
   end
...
function checkInit()
   ...
   if DEBUG_DETAIL then
      printDebug("React to Naval with Missile: checkInit")
   end
end
-----------------------------------------------
...
function check()
   if DEBUG_DETAIL_CHECK then
      printDebug(string.format("\n%.3f React to Naval with Missile; checking for threats.",
         vrf:getSimulationTime()))
   end
   if this:getEmbarkedOn():isValid() then
      printWarn(vrf:trUtf8("React to Naval with Missile: Cannot attack while embarked."))
      return false
   end
   ...
   if validWeaponList == nil then
      ...
      if next(validWeaponList) == nil then
         printWarn(vrf:trUtf8("React to Naval with Missile: No valid anti-surface weapons."))
   ...
   if next(readyWeapons) == nil then
      if weaponsReady then
         printWarn(vrf:trUtf8("React to Naval with Missile: No missiles ready."))
         weaponsReady = false
      end
      return false
   else
      if not weaponsReady then
         printWarn(vrf:trUtf8("React to Naval with Missile: Missiles ready."))
         weaponsReady = true
      end
   end
...
function init()
   if DEBUG_DETAIL then
      printDebug("React to Naval with Missile: init reaction")
   end
...
function tick()
   if DEBUG_DETAIL then
      printDebug(string.format("%.3f React to Naval with Missile: tick",
         vrf:getSimulationTime()))
   end
   ...
   if attackSubtaskId == -1 then
      ...
      if DEBUG_DETAIL then
         printDebug(string.format("\n%.3f React to Naval with Missile: attacking threat %s with %s",
