...
--  taskParameters.terrainClamp Type: Bool (yes/no) - Clamp path to terrain
--  taskParameters.heightAboveTerrain Type: Real Unit: meters - If clamping, add this offset to clamp result for final altitude of point.
...
function tick()
   if (job) then
      ...
      -- reduce points that are too close together
      local bvol = this:getBoundingVolume()
      local  boundingThreshold = math.sqrt((bvol:getForward()*2)*(bvol:getForward()*2)+(bvol:getRight()*2)*(bvol:getRight()*2)+(bvol:getUp()*2)*(bvol:getUp()*2))

      points2 = pathUtilReducePoints(points, boundingThreshold * pruneFactor, false)

      -- clamping?
      if (terrainClamp ==  true) then

         -- Try to clamp, spin while terrain is paging
         local clampedPoints
         local airPoints
         clampedPoints, valid = pathUtilMakePointsClampedToTerrain(points2, heightAboveTerrain, false, false)
         airPoints = clampedPoints

         if valid then
            airPoints, valid = pathUtilMakeAirRoute(clampedPoints, heightAboveTerrain)
         end

         points3 = airPoints

         -- Did the point clamping finish?  
         -- If not, need time to page in terrain.
         if not valid then
            return
         end
      else
