...
Csv.prototype.parse = function (data, fn) {
    ...
    try {
        ...
        var matches = null,
            strMatchedValue, matchedDelimiter;
...
Csv.prototype.parseFileSync = function (path, fn) {
    var self = this;
    var data = fs.readFileSync(path, 'utf8').toString();
    self.parse(data, fn);
}

Csv.prototype.mapFileSync = function (path, fn) {
    var self = this;
    this.parseFileSync(path, function (err, data) {
        self.map(data, fn);
    });
}